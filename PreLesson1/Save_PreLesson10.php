...
$PreLesson1_10 = isset($_POST['q10']) ? intval($_POST['q10']) : 0;

// ดึงคะแนนเก่าจากฐานข้อมูล
$sql_old_score = "SELECT PreLesson1 FROM stp3_scores WHERE stp3_name = '$stp3_name'";
...
if (mysqli_num_rows($result_old_score) > 0) {
    ...
    $old_score = $row['PreLesson1']; // คะแนนเก่า
} else {
    $old_score = 0; // หากไม่มีคะแนนเก่าในฐานข้อมูล
}

// คำนวณคะแนนรวม (บวกคะแนนเก่ากับคะแนนใหม่)
$total_score1_10 = $old_score + $PreLesson1_10;

// SQL อัปเดตข้อมูลคะแนนของผู้ใช้
$sql_update_score = "UPDATE stp3_scores SET PreLesson1 = '$total_score1_10' WHERE stp3_name = '$stp3_name'";

if (mysqli_query($conn, $sql_update_score)) {
    echo "✅ บันทึกข้อมูลสำเร็จ! คะแนนรวม: " . $total_score1_10;

    // เปลี่ยนเส้นทางไปหน้า Lesson1.1.html
    header("Location: ../Lesson1/Lesson1.1.php");
